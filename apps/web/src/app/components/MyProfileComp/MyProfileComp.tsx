...
import { CAuthUser, CCourse, CLesson } from "@/types/client";
import LoadingBarLoader from "../shared/LoadingBarLoader";

export const MyProfilePageComp = (): JSX.Element => {
    ...
    const [pendingLessons, setPendingLessons] = useState<number>(0);
    ...
    const completedCourseOfUser = (): void => {
        // Use the completedCourses field directly from user data
        const completedCoursesCount: number = authUser?.completedCourses?.length || 0;
        const enrolledCoursesCount: number = authUser?.enrolledCourses?.length || 0;
        const pendingCoursesCount: number = enrolledCoursesCount - completedCoursesCount;
        ...
        if (courses.length > 0) {
            ...
            courses.forEach((course: CCourse) => {
                ...
                course.lessons?.forEach((lesson: CLesson) => {
    ...
    const filteredCourses: CCourse[] =
        searchTerm?.trim() === ""
            ? courses.filter((course: CCourse) => authUser?.enrolledCourses?.includes(course._id))
            : courses.filter((course: CCourse) => {
                const isEnrolled = authUser?.enrolledCourses?.includes(course._id);
    ...
    const lessonProgress = (course: CCourse) => {
        const totalLessons: number = course?.lessons?.length || 0;
        const completedLessons: number = course?.lessons?.filter(
            (lesson) => lesson.status === "completed"
        ).length || 0;
        const progressPercentage: number = totalLessons > 0 ? (completedLessons / totalLessons) * 100 : 0;
    ...
    const handleUpdateUserInfo = async (): Promise<void> => {
        ...
        const phoneError: string | null | number = validatePhoneNumber((phoneNumber.toString()));
                ...
                <div className="w-full flex flex-col justify-center items-center  gap-6">
                    ...
                    <div className="w-full ">
                        {filteredCourses?.length == 0 ? (<div className="w-full h-full flex items-center justify-center">
                            ...
                            (<div className="w-full grid grid-cols-3 gap-3">{(filteredCourses || []).map((course: CCourse) => (
                                <Link
                                    key={course?._id}
...
export default MyProfilePageComp;
