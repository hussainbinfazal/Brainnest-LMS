...
export default function CourseIdPageComp({ initialCourse, initialReviews, allCategories, courseCategory, relevantCategoryCourses, instructorStats, userCourseStats, otherCoursesByInstructor, initialTopic, allLessons, allSections, userProgress, className }: CourseIdPageCompProps): React.JSX.Element {
  ...
  const likeCourse = async (): Promise<void> => {
    ...
    const currentUserCourse =
      useUserCourseStore
        .getState()
        .userCourseByCourseId[courseId];

    // if (!currentUserCourse) {
    //   console.log("4. This is currentUserCourse", currentUserCourse)
    //   return;
    // }
    console.log("2. LIKE COURSE FUNCTION CALLED")

    //Avoid duplicate request
    if (currentUserCourse?.isLiked) {
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        ...
        clientLogger.error("Error liking course", {
          status: error.response?.status,
          data: error.response?.data,
          message: error.message,
        });
      ...
      setUserCourseById(courseId as string, previousUserCourse);
        clientLogger.error("Error liking course", message);
    ...
    const unlikeCourse = async (): Promise<void> => {
      ...
      if (!currentUserCourse.isLiked) return

      // Save previous state for rollback
      const previousUserCourse = currentUserCourse;
      // params mai user id pass karni   hai
      setLiked(courseId as string, { //This zustand state updated
        isLiked: false,
        likedAt: null
      })
      try {
        const reponse = await axios.delete(`/api/dislikeCourse/${courseId}`);
        setUserCourseById(courseId as string, reponse.data.userCourse) // This is the updated user course state in compoent 
        toast.success("Course Disliked!");
      } catch (error: unknown) {
        setUserCourseById(courseId as string, previousUserCourse);
        clientLogger.error("Error disliking course", error);
        toast.error("Failed to dislike course");
      }
    };
    ...
    useEffect(() => {
      ...
      if (initialCourse && initialReviews.length > 0 && allCategories.length > 0 && courseCategory, relevantCategoryCourses.length > 0 && instructorStats && otherCoursesByInstructor && initialTopic && allLessons && allSections && userProgress) {
