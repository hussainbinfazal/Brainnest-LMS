...
interface CourseIdPageCompProps {
  ...
  otherCoursesByInstructor: CCourse[] | [];
  initialTopic: CTopic | null;
  allLessons: CLesson[] | [];
  allSections: CSection[] | [];
  uProgress: UserProgressResult | null;
  className?: string;
}
export default function CourseIdPageComp({ initialCourse, initialReviews, allCategories, courseCategory, relevantCategoryCourses, instructorStats, userCourseStats, otherCoursesByInstructor, initialTopic, allLessons, allSections, uProgress, className }: CourseIdPageCompProps): React.JSX.Element {
  const router = useRouter();
  const params = useParams();
  // console.log("This is user Progress", userProgress)
  ...
  const [reviews, setReviews] = useState<CReview[] | null>(initialReviews ?? null);
  const [userCourse, setUserCourse] = useState<CUserCourse | null>(userCourseStats ?? null)
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const isLiked = useUserCourseStore(
    (state) =>
      courseId
  ...
  const [topic, setTopic] = useState<CTopic | null>(initialTopic || null);
  const [userProgressData, setUserProgressData] = useState<CProgress | null>(uProgress?.currentProgress ?? null);
  const [userLessonsProgress, setUserLessonsProgress] = useState<CLessonProgress[] | []>(uProgress?.lessons ?? []);
  ...
  const toggleLikeCourse = async (): Promise<void> => {
    if (!user) {
    ...
    setLiked(courseId, {
      isLiked: shouldLike,
      likedAt: shouldLike ? new Date() : null,
    });
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        ...
        clientLogger.error(`Error ${shouldLike ? "liking" : "disliking"} course`, {
          status: error.response?.status,
          data: error.response?.data,
          message: error.message,
        });
      ...
      clientLogger.error(`Error ${shouldLike ? "liking" : "disliking"} course`, message);
      toast.error(`Failed to ${shouldLike ? "like" : "dislike"} course`);
    } finally {
  ...
  const handleAddToCart = async (courseId: string) => {
    ...
    try {
      ...
      clientLogger.error("Error adding course to cart", message);
  ...
  useEffect((): void => {
    const checkLessons = async (): Promise<void> => {
      if (userLessonsProgress.length && user) {
        for (const s of userLessonsProgress) {
          if (s._id) {
            await checkCompletedLesson(s._id);
          }
        }
      }
    };
  ...
  useEffect(() => {
    ...
    if (initialCourse && initialReviews?.length && allCategories.length > 0 && courseCategory, relevantCategoryCourses.length > 0 && instructorStats && otherCoursesByInstructor && initialTopic && allLessons && allSections && uProgress) {
  ...
  return (
    <div className={cn("relative w-full min-h-screen flex flex-col gap-6  py-18 pt-0 ", className)}>
      ...
      <div className="relative w-[90%] md:w-[90%] lg:w-[60%] bg-transparent mx-auto flex flex-col justify-start items-center h-full gap-6 min-h-screen pt-2">
        {(
          !isEnrolled && (
            <div className="absolute right-0 top-2 w-10 h-10 rounded-full flex items-center justify-center border border-gray-300 group cursor-pointer">
              {isLiked ? (
                <IoMdHeart
        ...
        {(
          isEnrolled && (
            <div
              className="absolute right-0 top-2 w-10 h-10 rounded-full flex items-center justify-center border border-gray-300 group cursor-pointer"
              onMouseEnter={() => {
                setIsViewInfo(true);
              }}
              onMouseLeave={() => {
            ...
            >
              <TbMessageUser className="text-2xl relative group-hover:scale-110" />
              {isViewInfo && (
        ...
        {(
          !isEnrolled && (
            <div className="absolute right-12 top-2 w-10 h-10 rounded-full flex items-center justify-center border border-gray-300 group cursor-pointer">
              {isAlreadyAdded ? (
                <BsCartCheckFill
                  aria-label="remove-from-cart"
                  className="text-xl  group-hover:scale-110 ease-in-out duration-400 transition-transform active:scale-90"
                  onClick={() => {
                    if (!user) {
                      router.push("/login");
                      toast.error("Please login first");
        ...
        {(
          <div className="absolute right-33 top-2 w-10 h-10  flex items-center justify-center  border-gray-300 group cursor-pointer">
            {!isEnrolled && (
              <Button
                size='default'
                variant="default"
                ...
                className="group-hover:scale-110 ease-in-out duration-400 transition-transform active:scale-90"
              >
                Buy Now
              </Button>
            )}
        ...
        <div className="flex gap-2 w-full   justify-start items-center">
          ...
          <span className="flex gap-2  items-center" data-testid="course-language">
            {(
              <IoGlobeOutline />
            )}
            {""}
        ...
        <div className="w-full h-30 flex flex-row justify-start items-center border-2 border-gray-300 rounded-lg mt-4">
          ...
          {(
            <span className="w-2/5 sm:w-1/5 flex flex-col gap-1 justify-center items-center wrap-break text-center">
              ...
              <p>{course?.totalEnrolledCount || 0} learners</p>
            </span>
          )}
        </div>
        {(
        ...
        {(
          <div className="w-full flex flex-col  justify-start items-start mt-4">
            {(
              <h3 className="text-2xl font-semibold px-2">Requirements</h3>
            )}
        ...
        {(user && coursesByInstructor.length > 0 &&
          ...
          (<div className="w-full flex flex-col  justify-between items-start mt-8 px-2">
            <div className="w-full">
              ...
              {coursesByInstructor.length > 0 && <div className="grid-cols-6 flex-1">
                <Carousel setApi={() => { }} opts={{}} plugins={[]} className="w-full">
                  <CarouselContent className=''>
                    {coursesByInstructor.length === 0 ? (
                      ...
                      (coursesByInstructor || []).map((course, index) => (
                        <CarouselItem
                          key={`${index}-${index}`}
