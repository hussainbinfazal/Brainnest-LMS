...
## August 17
Changes
-Added useUserCourse zustand store with types and states as a caching layer on client side to cache user's course releatd stats.
-Refractored User's Course Interface to match the updated items in  Controller and removed outdated old part to match the updated state
Added server as the single source of truth to match the data across app.
-Removed dead code that is unnecessary now
-Refractored client types to match the updated types
