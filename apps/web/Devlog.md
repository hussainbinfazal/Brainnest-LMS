...
## August 5
Changes
-Added reusable skelton components to improve ux
-Added turbopack to increase pace of development process.
-Added responsiveness in page component
-Tested redis caching, it is working.
-Refractored zustand stand to store the latest states
...
## August 7
Changes
-Refractored courseId Page, defined client types to match the updated api response
-Removed Unnecessary fields
-Added props based, data handling
-Cached data, such as userCourses, course and courseUserLiked to save unecessary api calls.
...
## August 16
Changes
-Added Progress Store to cache userProgress globally
-Added caching layer in job to cache heavy operations
-Added redisCache key of complete project in a single file
-Refractored Progress Controller and removed outdated old part to match the updated state
-Added Instructor stats section
-Removed dead code that is unnecessary now
-Refractored client types to match the updated types
...
## August 20
Changes
-Refractored form state and body format to match form schema on server and client side.
-Added new learner feedback component
...
## August 22
Changes
-Added User course api route with redis layer.
-Fetched & Synced user course on frontend and backend
-Refractored useUserCourseStore and synced user course with methods and states
...
## August 31
Changes
-I have written unit test with jest, to ensure there is no error in production.
-Added User progress api route with redis layer.
-Fetched & Synced user progress with previous user progress state on client and backend
-Refractored userUserProgressStore and synced user progress with methods and states in store with state
...
## August 3
Changes
-Added faceted Search on client side
-Added api route with mongodb aggregate pipeline with facets to fetch and process queries for faceted objects on client side.
-Added query route with pagination as params to manage courses on courses page perfectly.
-Refractored client side logic by passing main data as ssr props to reduce latency, and zustand state for caching,


- Add caching with zustand store to 
- lessons video processing with cloudinary SDK and sections logic on the client side 
-cart function on courseIdComp
...
## August 4
Changes
--Replaced client-side sidebar facet computation (looping over paginated course data in useEffect) with a MongoDB $facet aggregation — categories, languages, and levels computed server-side in one query, cached in Redis
--Fixed a subtle bug where filters and search were only ever operating on the current page's 6 courses instead of the full catalog — moved category/subcategory/language/level filtering into the Mongo query itself, with countDocuments and find sharing the same filter so pagination math stays correct
--Redesigned cache keys to encode every query parameter (page, filters) so different filter combinations don't collide in Redis
Cleaned up a pile of stale/dead useEffects and clarified when useCallback actually matters (only when something checks a function's identity, not "just in case")
--Traced a Mongoose 9 breaking change (FilterQuery → QueryFilter) that was silently failing TypeScript
--Designed a batch endpoint for fetching per-user course interaction records (UserCourse junction collection) instead of syncing a likedCourses array on the User document — same pattern I've been using for granular progress tracking
...
## August 7
Changes
--Added Skeleton component on loading state in all courses page
--Added responsiveness
--Refractored layout and matched the states with newly added fields.
--Added Updated Keys in universal redis key file.
--Refractored progress api route
...
## August 8
Changes
--Added UI changes in footer and header component.
--Added My Courses field for student enrolled courses.
--Added route for user's all courses with redis layer.
--Added all category & user all courses fetch and pass logic with ssr on homepage.
--Added All User Course setters and getters zustand for client caching.

- Add caching with zustand store to 
- lessons video processing with cloudinary SDK and sections logic on the client side 
-cart function on courseIdComp
-Update getSession with actual getSession (next-auth)
-- Completed
...
## August 9
Changes
--Added card in header component for user profile, user's enrolled courses, user liked courses.
--Refractored Liked Courses Page.
--Added route for user's all liked courses with redis layer.
--Added All User liked courses Course setters and getters zustand for client caching.
...
## August 10
Changes
--Added liked Courses API with query params(pagination).
--Refractored Liked Courses Page .
--Added All User liked courses Course setters and getters zustand for client caching.
--Added changes in UI, modified pagination component, Dislike button and states.
